...
class HtmlParserUtils {
  ...
  static List<Financial> parseFinancialTable(String htmlContent) {
    ...
    try {
      ...
      for (int i = 1; i < rows.length; i++) {
        ...
        final revenue = _parseFinancialValue(
          cells, columnMap['revenue'], 'revenue'
        );
        final profit = _parseFinancialValue(
          cells, columnMap['profit'], 'profit'
        );
        final cashFlow = _parseFinancialValue(
          cells, columnMap['cashFlow'], 'cashFlow'
        );
        final freeCashFlow = _parseFinancialValue(
          cells, columnMap['freeCashFlow'], 'freeCashFlow'
        );
        final margins = _parseFinancialValue(
          cells, columnMap['margins'], 'margins'
        );
      ...
      print('Error parsing financial HTML: $e');
      return [];
    }
  }
  ...
  static double? _parseFinancialValue(
    ...
    try {
      ...
      print('Error parsing value for $fieldName: $cellText - $e');
      return null;
    }
  }

  /// Parses a simple HTML table and returns structured data
  static Map<String, List<Map<String, String>>> parseGenericTable(String htmlContent) {
    if (htmlContent.isEmpty) return {};
    ...
    try {
      ...
      for (int tableIndex = 0; tableIndex < tables.length; tableIndex++) {
        ...
        final headerRow = rows.first;
        final headers = headerRow.querySelectorAll('td, th')
      ...
      print('Error parsing generic HTML table: $e');
      return {};
    }
  }
}
